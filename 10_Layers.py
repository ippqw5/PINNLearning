...
# %%
mylayer = layers.Lambda(lambda x:tf.math.exp(x))
mylayer(tf.range(5,dtype=tf.float32))
...
# %%
l1 = Linear(units = 10)
print(l1.built) #此时还未build ，即根据input_shape生成顺序参数

l1.build(input_shape=(None,10)) #此时已built，只有当self.built = True后，才可以训练。
print(l1.built)
...
model.add(Linear(units = 10,input_shape = (2,)))  
print("model.input_shape: ",model.input_shape)
print("model.output_shape: ",model.output_shape)
model.summary()
# 20 个 weights + 10 个 biases
